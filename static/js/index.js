    ...
    try {
        ...
        if (data.logged_in) {
            ...
            if (document.getElementById('sidebarAvatar')) {
                document.getElementById('sidebarAvatar').innerText = currentUser.username.substring(0, 2).toUpperCase();
            }
            ...
            if (document.getElementById('sidebarBalance')) {
                document.getElementById('sidebarBalance').textContent = (parseFloat(currentUser.balance) || 0).toLocaleString('vi-VN') + 'đ';
            }
            if (document.getElementById('sidebarTotalDeposit')) {
                document.getElementById('sidebarTotalDeposit').textContent = (parseFloat(currentUser.total_deposit) || 0).toLocaleString('vi-VN') + 'đ';
            }
            if (document.getElementById('sidebarUsedBalance')) {
                document.getElementById('sidebarUsedBalance').textContent = (parseFloat(currentUser.total_used) || 0).toLocaleString('vi-VN') + 'đ';
            }
            if (document.getElementById('sidebarDiscount')) {
                document.getElementById('sidebarDiscount').textContent = (currentUser.discount || 0) + '%';
            }
...
async function renderNavMenu() {
    try {
        ...
        // Keep the "Tất cả sản phẩm" link
        navMenu.innerHTML = `<li><a href="/" class="${currentPath === '/' ? 'active' : ''}" onclick="navigate('/', event)">Tất cả sản phẩm</a></li>`;
...
async function checkout() {
    ...
    try {
        ...
        if (data.success) {
            ...
            closeCheckout();
            // Reset button after successful purchase, as the modal is closed
            btn.disabled = false;
